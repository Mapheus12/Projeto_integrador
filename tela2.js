...
function preload() {
  ...
  setaEsquerda = loadImage('./RESOURCES/seta_esquerda.png');
  setaDireita = loadImage('./RESOURCES/seta_direita.png');
...
function draw() {
  ...
  let btnW = 120;
  let btnH = 60;
  let btn1X = width / 2 - 450 - btnW / 2;
  let btn1Y = height - 100;
  let btn2X = width / 2 + 450 - btnW / 2;
  let btn2Y = height - 100;

      // Checa se o mouse está sobre o botão esquerdo
    let hoverEsq = mouseX > btn1X && mouseX < btn1X + btnW && mouseY > btn1Y && mouseY < btn1Y + btnH;
    // Checa se o mouse está sobre o botão direito
    let hoverDir = mouseX > btn2X && mouseX < btn2X + btnW && mouseY > btn2Y && mouseY < btn2Y + btnH;


      // Desenha o Botão Esquerdo com Hover
    if (hoverEsq) {
      image(setaEsquerda, btn1X, btn1Y + 3, btnW, btnH); 
    } else {
      image(setaEsquerda, btn1X, btn1Y, btnW, btnH); 
    }
    
    // Desenha o Botão Direito com Hover
    if (hoverDir) {
      image(setaDireita, btn2X, btn2Y + 3, btnW, btnH);
    } else {
      image(setaDireita, btn2X, btn2Y, btnW, btnH);
    }
}
